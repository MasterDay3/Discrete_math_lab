...
## Функції
### Algoritm
`check_request(request)`: Перевіряє, чи можна додати компоненту до замовлення без конфліктів з уже вибраними. Повертає True, якщо сумісна, інакше False.\
`add_component(request)`: Додає компоненту до списку користувача, якщо вона існує, сумісна і ще не додана. Повертає True при успіху, False — якщо додати неможливо.\
`delete_component(request)`: Видаляє компоненту зі списку користувача. Повертає True, якщо видалено, або False, якщо її не було.\
`parse_dot_file(filename)`: Зчитує DOT-файл, очищає його від коментарів і повертає список ребер графа.\
`build_adjacency_list(edges)`: Створює список суміжності графа на основі ребер.\
`has_path(graph, start, target)`: Перевіряє наявність шляху між двома вершинами за допомогою DFS.\
`has_path_bfs(graph, start, target)`: Перевіряє наявність шляху між двома вершинами за допомогою BFS.\
`check_negative_paths(filename)`: Аналізує граф і перевіряє, чи існує шлях від вершини v до її заперечення -v. Виводить результат у консоль.
